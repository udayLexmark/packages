...
%ifvar parameters%
    ...
    <tr class="subheading2"><td colspan=2>Connection Copy Properties</td></tr>
             ...
             %else%
                <script>writeTDspan('rowdata-l'); swapRows();</script>
                    %ifvar resourceDomain -notEmpty%
                        <select name=".CPROP.%value systemName%" %ifvar systemName equals('loadBalancing')%onchange="handleLoadBalancingChange(this.form);"%endif%%ifvar systemName equals('connectionType')%onchange="handleConnectionTypeChange(this.form);"%endif%%ifvar systemName equals('sncMode')%onchange="handleSNCChange(this.form);"%endif%>
                            %loop resourceDomain%
