...
<SCRIPT LANGUAGE="JavaScript">
	...
	window.parent.hideSub(user,'%value notificationNodeName%');}
	...
	function queryUsers(){
		...
		if ( document.getElementById("provSelect") == null ) {
		  ...
		  prov1 = getProvider(document.getElementById("provSelect").selectedIndex);
		}
		
		var query = document.getElementById("query");
		
        if(is_csrf_guard_enabled && needToInsertToken) {
			createForm("htmlForm_subUserLookup", "subUserLookup.dsp", "POST", "BODY");
			setFormProperty("htmlForm_subUserLookup", "selectedOption", prov1);
			setFormProperty("htmlForm_subUserLookup", "query", encodeURIComponent(query.value));
            setFormProperty("htmlForm_subUserLookup", _csrfTokenNm_, _csrfTokenVal_);
			document.htmlForm_subUserLookup.submit();
        } else {
			window.location='subUserLookup.dsp?selectedOption='+prov1+'&query='+ encodeURIComponent(query.value);
		}
  ...
  //
  		...
  		icon.border=0;
  		icon.align='bottom';
  		icon.src='../WmRoot/icons/icon_user.gif';
...
<body style="border-spacing: 0; border-width:0">
	...
	if(navigator.userAgent.indexOf("Firefox")!=-1){
        ...
        <table  id="lookUpTable" width="250" style="cell-spacing:0 border-spacing: 0; border-collapse:collapse; border-width:0; border-style:solid; border-color:gray">
	<tr><th  class="heading" colspan=2 align="left">User Name</th></tr>

	%invoke wm.server.jndi:getSettings%
		<script language="javascript">
			server_dir_setting = "%value provider encode(javascript)%";
	...
	<style type="text/css" rel="stylesheet">
        %comment% don't use webMethods.css, since it will conflict with the hover color for the user "row" %endcomment%
	...
	%loop users%groupList[%value $index%]=new Array("%value name encode(javascript)%");%endloop%
    %endinvoke%
