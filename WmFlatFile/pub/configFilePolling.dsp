          ...
          else if(enabled == "true")
          {
            if(confirm("Port must be disabled so that you can edit these settings.  Would you like to disable the port?"))
            {
				if(is_csrf_guard_enabled && needToInsertToken){ 
					createForm("htmlform_filePolling_state_change", "configFilePolling.dsp", "POST", "BODY");
					setFormProperty("htmlform_filePolling_state_change", "listenerKey", "%value encode(javascript) listenerKey%");
					setFormProperty("htmlform_filePolling_state_change", "pkg", "%value pkg%");
					%ifvar listenerType%
						setFormProperty("htmlform_filePolling_state_change", "listenerType", "%value listenerType%");
					%endif%
					setFormProperty("htmlform_filePolling_state_change", "mode", "edit");
					setFormProperty("htmlform_filePolling_state_change", "disableport", "true");
					setFormProperty("htmlform_filePolling_state_change", _csrfTokenNm_, _csrfTokenVal_);
					document.htmlform_filePolling_state_change.submit();
				} else {
      ...
      function verify(listenerKey) {
        ...
        */
        ...
        if (( e != null ) && ( e != "" ) && !isblank(e)) {
			...
			if ((isNaN(num_files)) || (num_files == 0) || (num_files < -1)) {
