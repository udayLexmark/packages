...
%onerror%
%endinvoke%
<BODY onLoad="setNavigation('ListListenerNotifications.dsp', '%value helpsys%', 'foo');%ifvar lt8%%else%showHideFilterCriteria('searchListenerNotificationName');%endif%">
        %ifvar lt8%%else%<form name="form" action="ListListenerNotifications.dsp" method="POST">%endif%
<table width="100%">  
    <tr>
       	<td class="menusection-Adapters" colspan=5>Adapters &gt; %value displayName% &gt; Listener Notifications</TD>
    </tr>
    %comment%
    ...
    %ifvar notificationDataList -notempty%
        %loop notificationDataList%
            ...
            %case 'yes'%
                <img src="/WmRoot/images/green_check.gif" height=13 width=13 alt="Enabled" border=0>Yes</td>
            %case 'no'%
                 <img src="/WmRoot/images/blank.gif" border=0 alt="Disabled">No</td>
            %case 'pending'%         
                 <img src="/WmRoot/images/blank.gif" border=0 alt="Pending disable">Pending disable</td>
            %endswitch%            
            </tr>
        %endloop%
    %else%
        <TR><TD class="message" colspan=3>No Listener Notifications found</TD></TR>
    %endif%
    %onerror% 
