...
%invoke LXKAdmin.server.query:getTaskList%
<!-- Server tasks table -->
<table width="100%">
...
<SCRIPT LANGUAGE="JavaScript">
  ...
  function onAdd() {
   ...
   // If latenessAction is NOT "Run immediately", then Check for Lateness time
	...
	{
		if ( document.schedule.lateness.value == "")
		{
			alert("Specify lateness (in minutes)");
			document.schedule.lateness.focus();
			return false;
		}
	}
    }
  ...
  <form name="schedule" action="scheduler.dsp" method="POST">
    ...
    <tr>
      ...
      <td>
        ...
        %invoke LXKAdmin.server.query:getUserTaskList%

        <script type="text/javascript">
          var trgt = "%value target%";
        ...
        <tr>
          ...
          <td class="oddrow-l" colspan=5><input name="description" size=40 value="%value description%"></input></td>
        </tr>
        <tr>
          <td class="evenrow" colspan=2>folder.subfolder:service</td>
          <td class="evenrow-l" colspan=5><input name="service" size=40 value="%value name%"></input></td>
        </tr>
	...
	<SCRIPT>
	  ...
	  function callback(val){
        ...
        <tr>
          ...
          <input name="runAsUser" size=12 value="%value runAsUser%"></input>
	...
	<script type="text/javascript" src="subUserLookup.js"></script>
	   <a class="submodal" href="subUserLookup.dsp"><img border=0 align="bottom" src="icons/magnifyglass.gif"/></a>
        ...
        <tr>
          <td class="evenrow" colspan=2>Cluster Target Node</td>
          <td class="evenrow-l" colspan=5>

		  %invoke LXKAdmin.server.query:getClusterSettings%
          ...
          <SELECT NAME="target">
            ...
            %loop hosts%
            <OPTION value="%value logicalServerName%" 
            %ifvar logicalServerName vequals(../target)% selected  %endif%
            >%value logicalServerName%</OPTION>
            %endloop%
          </SELECT>
		  %else%(Cluster unavailable)%endif%
		...
		<tr>
		  ...
		  <td class="evenrow-l" colspan=4 rowspan=2>if more than <input name="lateness" size=12 value="%value lateness%"></input> minutes late.</td>
		</tr>
		<tr>
        ...
        <tr>
          ...
          <td class="oddrow-l" colspan=5><input name="lateness" size=12 value="%value lateness%"></input> minutes</td>
        <tr>
...
%ifvar type equals(once)%<input name="type" type="hidden" value="once" >%endif%
            <tr>
              ...
              <input name="date" size=12 value="%value start-date%"> YYYY/MM/DD</input>
              </td>
            </tr>
            <tr>
              <td class="evenrow" colspan=1>Time</td>
              <td class="evenrow-l" colspan=6>
              <input name="time" size=12 value="%value start-time%"> HH:MM:SS </input>
              </td>
            </tr>
          %case 'repeat'%
        ...
        %endif%
          ...
          <tr>
            <td class="action" colspan=7>
              <input type="hidden" id="oid" name="oid" value="%value oid%"></input>
  ...
  %endif%
    ...
    %case 'cancel'%
      ...
      %invoke wm.server.schedule:cancelUserTask%
        <script>document.location.replace("scheduler.dsp?message=Cancelled%20Task%20ID:%20%value -urlencode oid%&filterTask=%value filterTask%&statusActive=%value statusActive%&showAll=true");</script>
      %onerror%
    ...
    %case 'suspend'%
      ...
      %invoke wm.server.schedule:suspendUserTask%
       %ifvar taskSuspended equals('true')%
	      <script>document.location.replace("scheduler.dsp?message=Suspended%20Task%20ID:%20%value -urlencode oid%&filterTask=%value filterTask%&statusActive=%value statusActive%&showAll=true");</script>
	  %endif%    
    ...
    %case 'wakeup'%
      ...
      <script>document.location.replace("scheduler.dsp?message=Woke%20Up%20Task%20ID:%20%value -urlencode oid%&filterTask=%value filterTask%&statusActive=%value statusActive%&showAll=true");</script>
      %onerror%
    ...
    %case 'Pause Scheduler'%
      ...
      <script>document.location.replace("scheduler.dsp?message=The Scheduler has been paused.  No scheduled tasks will be initiated until the Scheduler is resumed.");</script>
      %onerror%
      <script>document.location.replace("scheduler.dsp?message=Error pausing Scheduler%");</script>
      %endinvoke%

    %case 'Resume Scheduler'%
      <!-- =============== Pause Scheduler =============== -->
      %invoke wm.server.schedule:resumeScheduler%
      <script>document.location.replace("scheduler.dsp?message=The Scheduler has been resumed.");</script>
      %onerror%
      <script>document.location.replace("scheduler.dsp?message=Error resuming Scheduler%");</script>
      %endinvoke%
    ...
    <tr>
      <td colspan=2>
	<ul>
          %invoke LXKAdmin.server.schedule:getSchedulerState%
          %ifvar state equals('running')%
           %comment% <li><a href="scheduler.dsp?schaction=Pause Scheduler" onclick="return confirmPause();">Pause Scheduler</a> (currently Running)</li> %endcomment%
          %else%
           %comment% <li><a href="scheduler.dsp?schaction=Resume Scheduler" onclick="return confirmResume();">Resume Scheduler</a> (currently Paused)</li> %endcomment%

		  %endif%
		  %endinvoke%
          <li><a href="scheduler.dsp?mode=sys">View System Tasks</a></li>
          %comment% <li><a href="scheduler.dsp?mode=useradd">Create a scheduled task</a></li> %endcomment%
	   <!-- 1-1RA0AX: Filter Services functionality -->
	    <li id="showfew" name="showfew"><a href="javascript:showFilterPanel(true)">Filter Services</a></li>
	    <li style="display:none" id="showall" name="showall"><a href="scheduler.dsp">Show All Services</a></li>
	    ...
	    <DIV id="filterContainer" name="filterContainer" style="display:none;padding-top=2mm;">
				  ...
				  <input type="text" name="filterTask" id="filterTask" value="%value filterTask%" onKeyPress="filterServices(event)"/>

				<IMG SRC="images/blank.gif" height=20 width=60>
			  ...
			  <IMG SRC="images/blank.gif" height=30 width=40>
      ...
      <td>
        <table class="tableView" width=100% id="head" name="head">
          ...
          <tr>
              ...
              %ifvar parentID%%value% 
               	<script>writeTD("childrow");</script>
	              	<font color = Blue>%value name%</font></td>
			  %else%
              	<script>writeTD("rowdata-l");</script>
    	        	<font color = Blue>%value name%</font></td>
			  %endif%
			...
			%ifvar schedState equals('expired')%
					...
					%else%
	              		<script>writeTD("rowdata-l");</script>
	    		  	%endif%
                  	<font color = Blue>Expired</font></td>
		    %else%
              %ifvar execState equals('suspended')%
					...
					%else%
	              		<script>writeTD("rowdata-l");</script>
	    		  	%endif%
                    <font color = Blue>Suspended</font></td>
              %else%
					...
					%else%
	              		<script>writeTD("rowdata-l");</script>
	    		  	%endif%
                    <font color = Blue>%ifvar execState equals('running')%Running%else%Active%endif%</font></td>
			  %endif%
            %endif%

            <script>writeTD("rowdata");</script>
                 <img src="icons/delete.gif" border=0></td>
          </tr>
		  <!--tr>
