        ...
        <script>
            ...
            function checkClaims(form, name, values)
            {
               ...
               var username = document.getElementById("name").value;
                   ...
                   var rc = checkLegalUserName(claimname);
                   var val = checkLegalUserName(claimvalues);

                    if (rc != "") {
                        alert("Claim name: " + claimname + " contains illegal character \"" + rc + "\"");
                        return false;
                    }

                    if (containsNonAscii(claimname)) {
                        alert("Claim name: " + claimname + " contains non-ASCII character(s)");
                        return false;
                    }
                    
                    if (val != "") {
                          alert("Claim values: " + claimvalues + " contains illegal character \"" + val + "\"");
                        return false;
                    }

                    if (containsNonAscii(claimvalues)) {
                        alert("Claim values: " + claimvalues + " contains non-ASCII character(s)");
                        return false;
                    }
            ...
            <tr>
                <td colspan=4>
                    <ul class="listitems">
                        <li class="listitem">
						<script>
						createForm("htmlform_users_provider_addremove_claims", "users-provider-addremove-claims.dsp", "POST", "BODY");
						setFormProperty("htmlform_users_provider_addremove_claims", "name", "%value name%");
						setFormProperty("htmlform_users_provider_addremove_claims", "provider", "%value provider%");
						</script>
						<script>getURL("users-provider-addremove-claims.dsp?name=%value name%&provider=%value provider%","javascript:document.htmlform_users_provider_addremove_claims.submit();","Return to Add and Remove OpenID Provider Users Claims");</script></li>
                    </ul>
                </td>
            </tr>
            <tr>
            ...
            <tr>
                    ...
                    <form id="addidtokenclaim" name="addidtokenclaim" method="POST" action="users-provider-addtoken-claims.dsp">
                        ...
                        <table class="tableView" width="75%">
                            ...
                            <tr>
                              <td nowrap valign="top" class="oddrow" width="25%"><label for="tokenclaimname">Claim Name<label></td>
                              %ifvar action equals('editidtokenclaim')%
                              <td class="oddrow-1">%value tokenclaimname encode(html)%
                              %else%
                              <td class="oddrow-1">
                              ...
                              <tr>
                              <td class="oddrow"><label for="tokenclaimvalues">Enter Claim values separated by commas</label></td>
                                %ifvar action equals('editidtokenclaim')%
                                  <td class="oddrow-1"><textarea style="width:100%;" wrap="off" rows="5" name="tokenclaimvalues" id="tokenclaimvalues" cols="20">%value tokenclaimvalues encode(html)%</textarea></td>
                                %else%
