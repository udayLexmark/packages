<html>
    ...
    <body onLoad="setNavigation('package-list.dsp', 'doc/OnlineHelp/wwhelp.htm?context=is_help&topic=IS_Packages_LockDetailsScrn');">
        <table width="100%">
            ...
            <tr>
                <td colspan=2>
                    <ul class="listitems">
							<script>
							createForm("htmlform_return_url", "%value returnURL encode(url)%", "POST", "BODY");
							</script>
							<li>
							<script>getURL("%value returnURL encode(url)%", "javascript:document.htmlform_return_url.submit();", "Return to %value returnName encode(html)%");</script>
							</li>
<!--
                            %switch LOCK_STATUS%
                                %case '2'%
