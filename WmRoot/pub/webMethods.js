...
function handleBreadCrumbs(sURL) {
	var params = parseQueryString();
	if (params["hideBreadcrumbs"]) {
		addHideBreadCrumbsQueryParam(sURL);
	}
}

function addHideBreadCrumbsQueryParam(sURL) {
	if (sURL.includes("?")) {
		document.write('&hideBreadcrumbs=true');
	}
	else {
		document.write('?hideBreadcrumbs=true');
	}
