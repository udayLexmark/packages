<HTML>
  <HEAD>
    ...
    function checkEverything()
    {
      if (!verifyRequiredNonNegNumber('logform', 'numlines'))
       ...
       var numEntered = document.getElementById("numlines").value;
		...
		{
			alert("Number of lines to display must be between 1 and 65535.");
			return false;
		}
    ...
    function refreshSearch() {
        ...
        {
            if(vin != null && !vin.closed) {
				return;
			}
			var url = 'calendar.dsp';
			url += '?month='+thisMonth;
			url += '&year='+thisYear;
			url += '&date='+thisDate;
			url += '&which='+escape(which);
			//window.open().document.write(url);
			try {
			var xmlHttp;
			...
			xmlHttp.onreadystatechange = function () {
				if(xmlHttp.readyState == XMLHttpRequest.DONE && xmlHttp.status == 200) {
					//alert("Getting the response" + xmlHttp.responseText);
					 vin = window.open( "", "calendar", "width=600,height=350,top=50,left=50,resizable=yes" )
					
					vin.document.write(xmlHttp.responseText);
  ...
  <BODY onLoad="setNavigation('log-audit-recent.dsp', 'doc/OnlineHelp/wwhelp.htm?context=is_help&topic=IS_Logs_AuditScrn');">
      ...
      %rename ../../qfromdate qfromdate -copy%
          ...
          <TD colspan=2>
            <TABLE class="tableView">
                  <TR>
                  %ifvar logdate%
                    <TD colspan=14 class="heading">Service Log Entries as of %value logdate encode(html)%</TD>
                  %else%
                  ...
                  <TR>
                     ...
                     <SCRIPT>writeTDnowrap("row-l");</SCRIPT>%value ServiceName encode(html)%</TD>
                     <SCRIPT>writeTDnowrap("row-l");</SCRIPT>%value Resubmittable encode(html)%</TD>
                     <SCRIPT>writeTDnowrap("row-l");</SCRIPT>%value Status encode(html)%</TD>
                     <SCRIPT>writeTDnowrap("row-l");</SCRIPT>%value Duration encode(html)%</TD>
                     <SCRIPT>writeTDnowrap("row-l");</SCRIPT>%value ErrorMessage encode(html)%</TD>
