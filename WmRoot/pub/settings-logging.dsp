  ...
  <HEAD>
    ...
    function onEdit ()
    {
      ...
      if(is_csrf_guard_enabled && needToInsertToken) {
		  createForm("htmlform_settings_logging", "settings-logging.dsp", "POST", "BODY");
		setFormProperty("htmlform_settings_logging", "doc", "edit");
        setFormProperty("htmlform_settings_logging", _csrfTokenNm_, _csrfTokenVal_);
		document.htmlform_settings_logging.submit();
      } else {
		  document.location.replace("settings-logging.dsp?doc=edit");
	  }
...
%invoke wm.server.query:getSettings%
		...
		<script>
		createForm("htmlform_settings_auditing1", "settings-auditing.dsp", "POST", "BODY");
		</script>
       %ifvar ../doc equals('edit')%
                    ...
                    <TD colspan=2>
                      <ul class="listitems">
						...
						<script>
						createForm("htmlform_settings_logging1", "settings-logging.dsp", "POST", "BODY");
						</script>
                        <LI>
						<script>getURL("settings-logging.dsp","javascript:document.htmlform_settings_logging1.submit();","Return to View Server Logger Details")</script></li>
                      </UL>
                    </TD>
                   </TR>
        ...
        %else%
            <FORM name="logform" action="stats-getlog.dsp" method="POST">
              ...
              <TR>
                <TD colspan=2>
                  <ul class="listitems">
                    <li class="listitem"><script>getURL("settings-auditing.dsp","javascript:document.htmlform_settings_auditing1.submit();","Return to Logger List")</script></li>
 ...
 <TD colspan="3" style="border-collapse: collapse; padding: 0; margin: 0; border: none">
  <TABLE id="Default" style="border-collapse: collapse; padding: 0; margin: 0; border: none">  
            ...
            <TR class="subheading2">
               <TH CLASS="oddcol-l">Facility</TH>
               <TH CLASS="oddcol">Logging Level</TH>

            </TR>
        <TR>
                <TD CLASS="evenrowdata-l">
                    <A onClick="toggleNode(this)" href="#"><img id="imgDefault" src="images/minus.gif">Default</A>
                </TD>
         ...
         %loop Components% 
	        <TR title="%value CompCode encode(htmlattr)%">
                    <SCRIPT>writeTD('rowdata-l');</SCRIPT>
                        <A onClick="toggle(this,'%value CompCode encode(javascript)%');" href="#"><img id="img%value CompCode encode(htmlattr)%" src="images/plus.gif"><label for="select_%value CompCode encode(htmlattr)%">%value CompDesc encode(html)%</label></A>
                    </TD>
                    <SCRIPT>writeTD('rowdata');swapRows();</SCRIPT>
            %ifvar ../../doc equals('edit')%
               %ifvar Threshold vequals(../DebugLevel)%
   		        <SELECT title="%value CompDesc encode(htmlattr)%" id="select_%value CompCode encode(htmlattr)%" onfocus="beforeChange(this.selectedIndex)" onchange="commonChange(this);afterChange(this)" style="color:#657383;width:80px" >
               %else%
   		        <SELECT title="%value CompDesc encode(htmlattr)%" id="select_%value CompCode encode(htmlattr)%" onfocus="beforeChange(this.selectedIndex)" onchange="commonChange(this);afterChange(this)" style="font-weight:bold;width:80px">		        
               %endif%
