...
<tbody id="providerJMSDiv">
  <!-- Provider JMS - start -->
    %ifvar ../../action equals('view')%
      <TR>
        ...
        %ifvar jmsTriggerName matches('wm.namespace.ws:wseTrigger_*')%
				...
				setFormProperty("htmlform_endpoint_trigger_details", "triggerName", "wm.namespace.ws:wseTrigger_%value ../../alias encode(url)%");
				</script>
				<A HREF="javascript:document.htmlform_endpoint_trigger_details.submit();">WS Endpoint Trigger: %value ../../alias encode(html)%</A></TD>
        %else%
    ...
    %else%
      ...
      <TR>
        <TD class="oddrow"><label for="jmsTriggerName">JMS Trigger Name</label></TD>
        <TD class="oddrow-l">
          <SELECT NAME="jmsTriggerName" id="jmsTriggerName" onChange="triggerChanged(this.value)" style="width:40%">
    ...
    %endif% 
  <TR>
    <TD class="evenrow"><label for="deliveryMode">Delivery Mode</label></TD>
    <TD class="evenrow-l">
    %ifvar ../../action equals('view')%
			%value deliveryMode encode(html)%
    %else%
      <SELECT NAME="deliveryMode" id="deliveryMode" style="width:40%">
  ...
  <TR>
    <TD class="oddrow"><label for="timeToLive">Time to Live</label></TD>
    <TD class="oddrow-l">
    %ifvar ../../action equals('view')%
			%value timeToLive encode(html)%
    %else%
			<INPUT NAME="timeToLive" id="timeToLive" VALUE="%value timeToLive encode(htmlattr)%" style="width:40%">
    %endif% 
    </TD>
  </TR>
  <TR>
    <TD class="evenrow"><label for="priority">Priority</label></TD>
    <TD class="evenrow-l">
    %ifvar ../../action equals('view')%
  ...
  <TR>
    <TD class="oddrow"><label for="replyToDestName">Reply To Name</label></TD>
    <TD class="oddrow-l">
      %ifvar ../../action equals('view')%
  ...
  <TR>
    <TD class="evenrow"><label for="replyToDestType">Reply To Type</label></TD>
    <TD class="evenrow-l">
      %ifvar ../../action equals('view')%
 ...
 <!-- <TR>
  ...
  -->
  <TR>
    <TD class="oddrow"><label for="includeConnFactoryName">Include Connection<br> Factory Name</label></TD>
    <TD class="oddrow-l">
      %ifvar ../../action equals('view')%
      ...
      %else%
        ...
        <INPUT TYPE="checkbox" id="includeConnFactoryName" onchange="checkboxChangeListener(this, 'includeConnFactoryName');" %ifvar includeConnFactoryName equals('false')%%else%checked%endif% />
      %endif% 
    </TD>
  </TR>
  <TR>
    <TD class="evenrow"><label for="includeJNDIParams">Include JNDI Parameters</label></TD>
    <TD class="evenrow-l">
      %ifvar ../../action equals('view')%
      ...
      %else%
        ...
        <INPUT TYPE="checkbox" id="includeJNDIParams" onchange="checkboxChangeListener(this, 'includeJNDIParams');" %ifvar includeJNDIParams equals('true')%checked%endif% />
      %endif% 
    </TD>
  </TR>
<!-- Provider JMS - end -->
...
<TD class="oddrow">Connect Using</TD>
  <TD class="oddrow-l">
    ...
    %else%
      <INPUT TYPE="radio" NAME="jmsAliasType" id="jmsAliasType1" VALUE="JNDI" %ifvar jmsAliasType equals('JMS')%%else%checked%endif% onclick="toggleJMSorJNDI()"><label for="jmsAliasType1">JNDI Properties</label></INPUT>
      <INPUT TYPE="radio" NAME="jmsAliasType" id="jmsAliasType2" VALUE="JMS" %ifvar jmsAliasType equals('JMS')%checked%endif% onclick="toggleJMSorJNDI()"><label for="jmsAliasType2">JMS Connection Alias</label></INPUT>
    %endif% 
  </TD>
</TR>
</tbody>
...
%else%
  ...
  <TR>
    <TD class="evenrow"><label for="jndiProvAlias">JNDI Provider Alias</label></TD>
    <TD class="evenrow-l">
      <SELECT NAME="jndiProvAlias" id="jndiProvAlias" style="width:40%">
        %loop aliasDataAry%
...
%endif%
<TR>
  <TD class="oddrow"><label for="connFactoryName">Connection Factory Name</label></TD>
  <TD class="oddrow-l">
    %ifvar ../../action equals('view')%
			%value connFactoryName encode(html)%
    %else%
			<INPUT NAME="connFactoryName" id="connFactoryName" VALUE="%value connFactoryName encode(htmlattr)%" style="width:40%"/>
    %endif% 
  </TD>
</TR>
...
%else%
  ...
  <TR>
    <TD class="evenrow"><label for="jmsConnAlias">JMS Connection Alias</label></TD>
    <TD class="evenrow-l">
      <SELECT NAME="jmsConnAlias" id="jmsConnAlias" style="width:40%">
        %loop aliasDataList%
