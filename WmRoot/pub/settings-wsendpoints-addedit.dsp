...
<HEAD>
	...
	function kerberosAuthFn(act, authType) {
	
		if('view' == act && 'kerberos' != authType) {
			hideRows("kerberosPropsTransportDiv");
			hideRows("kerberosPropsSPNFormatConsumerTransportDiv");
		    hideRows("kerberosPropsSPNTransportDiv");
			
		}
		if('edit' == act && 'kerberos' != authType ){
			hideRows("kerberosPropsTransportDiv");
			hideRows("kerberosPropsSPNFormatConsumerTransportDiv");
		    hideRows("kerberosPropsSPNTransportDiv");
		}
    ...
    function editParamRow(action, type, tablePrefix, paramValue) 
    {
		...
		addElement.setAttribute('text','add');
        if(navigator.appName == "Microsoft Internet Explorer") {
		...
		deleteElement.setAttribute('text','delete');
        if(navigator.appName == "Microsoft Internet Explorer") {
    ...
    function editResponseMapRow(action, addressVal, msgAddressingAliasVal, transportTypeVal)
    {
        ...
        addElement.setAttribute('type', 'button');
        addElement.setAttribute((document.all ? 'className' : 'class'), "addButton");
		addElement.setAttribute('title','Add');
        if(navigator.appName == "Microsoft Internet Explorer") {
    ...
    */
    ...
    function confirmEdit()
    {
        %ifvar transportType equals('JMS')%
            ...
            %ifvar type equals('consumer')%
				if (!confirmPassword(document.addform.messagePassword.value, document.addform.retype_messagePassword.value, "Message")) {
					return false;
				}
			...
			if (document.addform.messageUsernameTokenTTL.value != "" && !isPositiveInteger(document.addform.messageUsernameTokenTTL.value)) {
                alert ("The 'Username Token TTL' must be a positive integer.");
                return false;
            }

			if (document.addform.messageUsernameTokenFutureTTL.value != "" && !isPositiveInteger(document.addform.messageUsernameTokenFutureTTL.value)) {
                alert ("The 'Username Token Future TTL' must be a positive integer.");
                return false;
            }
    ...
    function confirmAdd() {
        ...
        if(document.addform.transType[2].checked) {
            ...
            if ((document.addform.alias.value == "")) {
				...
				if( document.addform.type.value  == "provider") {
					if ((document.addform.host.value == "") || (document.addform.port.value == "")) {
						alert ("You must specify host and port for the Provider web service endpoint.");
						return false;
					}
				...
				if ( document.addform.type.value  == "addressing") {
					if(!confirmPassword(document.addform.transportPassword.value, document.addform.retype_transportPassword.value, "Transport")) {
						return false;
					}
				}
    ...
    function hideAllToBegin() {
		...
		hideRows("kerberosPropsSPNFormatDivProvider");
		hideRows("kerberosPropsSPNFormatDivConsumer");
		hideRows("kerberosPropsSPNDiv");
		hideRows("kerberosPropsTransportDiv");
		hideRows("kerberosPropsSPNFormatConsumerTransportDiv");
		hideRows("kerberosPropsSPNTransportDiv");
    }
    ...
    function showWSSecurityProperties(type) {
        ...
        if (type == "provider")
        {
			...
			displayRows("usernameTokenMsgDiv");
    ...
    function toggleProperties(caller) {
        ...
        if(provider) {
            ...
            if(http) {
				...
				toggleKerberosPropertiesAtTransport(document.getElementById('authType').value);
            } else if (https) {
                document.getElementById('httpTransportHeader').innerHTML = "HTTPS";
                document.getElementById('httpTransportHeaderOpt').innerHTML = "&nbsp;(Optional)";
                displayRows("transportHTTPDiv");
        ...
        {    
			...
			if(document.getElementById('authType').value == 'kerberos')
			{
		...
		var authTypedropdown = document.getElementById("authType");
		if(consumer) {
			if(authTypedropdown.options.length == 3)
			{
			var option = document.createElement("OPTION");
       		option.text = "Kerberos";
       		option.value = "kerberos"; 
			authTypedropdown.options.add(option);
			}				
       		
		} else {
			if(authTypedropdown.options.length == 4) {
				authTypedropdown.remove(3);
			}
		}
    ...
    function toggleKerberosProperties(){
        ...
        if(https){
			...
			if(provider) {
				displayRows("kerberosPropsSPNFormatDivProvider");
				hideRows("kerberosPropsSPNFormatDivConsumer");
			} else {
				hideRows("kerberosPropsSPNFormatDivProvider");
				displayRows("kerberosPropsSPNFormatDivConsumer");
			}
			displayRows("kerberosPropsSPNDiv");
        }
        else{
            hideRows("kerberosPropsDiv");
			hideRows("kerberosPropsSPNFormatDivProvider");
	...
	function respondToClientAuthChange(cAuth) {	
        toggleKerberosPropertiesAtTransport(cAuth);
	}
	function toggleKerberosPropertiesAtTransport(cAuth){		
		var cAuth2 = cAuth;	
		if (cAuth2 != null && (cAuth2 == 'kerberos'))
		{
		   var authTypedropdown = document.getElementById("authType");
			if(authTypedropdown.options.length == 4 && authTypedropdown.options[3].value == cAuth2) {
				authTypedropdown.options[3].selected=true;
			}
		   displayRows("kerberosPropsTransportDiv");
		   displayRows("kerberosPropsSPNFormatConsumerTransportDiv");
		   displayRows("kerberosPropsSPNTransportDiv");
		} else {
			hideRows("kerberosPropsTransportDiv");
			hideRows("kerberosPropsSPNFormatConsumerTransportDiv");
			hideRows("kerberosPropsSPNTransportDiv");
  ...
  %ifvar action equals('edit')%
    ...
    %else%
        ...
        %endif% 
    %endif% 
    setNavigation('settings-wsendpoints.dsp', 'doc/OnlineHelp/wwhelp.htm?context=is_help&topic=IS_Settings_WebServiceAliasEditScrn');">
  %else%
      %ifvar action equals('view')%
        ...
        %else%
            ...
            %endif%
        %endif%;
	  	changeColumnProperties('%value type encode(javascript)%');setNavigation('settings-wsendpoints.dsp', 'doc/OnlineHelp/wwhelp.htm?context=is_help&topic=IS_Settings_WebSvcAliasDetailsScrn');">
      %else%
        <BODY onLoad="setupKeystoreData();
        togglePropertiesAtOnLoad('provider', 'HTTP'); 
        setNavigation('settings-wsendpoints.dsp', 'doc/OnlineHelp/wwhelp.htm?context=is_help&topic=IS_Settings_WebServiceAlias');">
      %endif%
  %endif%
      ...
      <TR>
        <TD colspan="2">
          <ul class="listitems">
			...
			<script>getURL("settings-wsendpoints.dsp", "javascript:document.htmlform_settings_wsendpoints.submit();", "Return to Web Services");</script>
              ...
              %ifvar action equals('view')%
				...
				setFormProperty("htmlform_settings_wsendpoints_addedit", "action", "edit");
				setFormProperty("htmlform_settings_wsendpoints_addedit", "alias", "%value alias %");
				setFormProperty("htmlform_settings_wsendpoints_addedit", "type", "%value type %");
				setFormProperty("htmlform_settings_wsendpoints_addedit", "transportType", "%value transportType %");
				</script>
				<script>getURL("settings-wsendpoints-addedit.dsp?action=edit&alias=%value alias encode(url)%&type=%value type encode(url)%&transportType=%value transportType encode(url)%", "javascript:document.htmlform_settings_wsendpoints_addedit.submit();", "Edit Web Service Endpoint Alias");</script>
...
%else%
    ...
    %else%
        ...
        <tbody id="executeACLDiv">
            <TR>
                <TD class="oddrow"><label for="executeACL">Execute ACL</label></TD>
                <TD class="oddrow-l">%invoke wm.server.access.adminui:aclList%
                    <SELECT name="executeACL" id="executeACL">
                    %loop aclgroups%
                        <OPTION
                        %ifvar ../executeACL vequals(name)%selected%endif%
						%ifvar ../executeACL%%else%%ifvar name equals('Internal')%selected%endif%%endif% value="%value name encode(htmlattr)%">%value name encode(html)%</OPTION>
